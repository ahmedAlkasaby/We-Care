        ...
        <div class="card-body pt-12">
            <div class="user-avatar-section">
                <div class=" d-flex align-items-center flex-column">
                    @if(!$case->user->image)
                    <img class="img-fluid rounded mb-4" src="{{url(" admin/assets/img/avatars/1.png")}}"
                        height="120" width="120" alt="User avatar">
                    @else
                    <img class="img-fluid rounded mb-4" src="{{ asset('uploads/' . $case->user->image)}}"
                        height="120" width="120" alt="User avatar">
                    @endif
            ...
            <div class="info-container">
                <ul class="list-unstyled mb-6">
                    ...
                    <li class="mb-2">
                        ...
                        <span>{{$case->get_price()}}</span>
                    </li>
                    <li class="mb-2">
                        <span class="h6">@lang("site.price_raised"):</span>
                        <span>{{$case->get_price_raised()}}</span>
                    </li>
                    <li class="mb-2">
                ...
                <div class="d-flex flex-column gap-2">
                    ...
                    @else
                    <button disabled class="edit-btn btn btn-primary waves-effect waves-light" href="{{route("cases.edit",["case"=>$case->id])}}">
                        <i class="ti ti-pencil me-1"></i> @lang('site.Edit')
                    </button>
                    @endif

                    @if (auth()->user()->hasPermission('transfers.store') && $case->this_case_need())
                    <button data-bs-toggle="modal" data-bs-target="#caseTransfer_{{ $case->id }}" class="edit-btn btn btn-success waves-effect waves-light" href="{{route("cases.edit",["case"=>$case->id])}}">
                        <span><i class="fa-solid fa-money-bill-transfer me-2"></i> <span
                            class="d-none d-sm-inline-block">@lang('site.transfer')</span></span>
                    </button>
                    @else
                    <button disabled data-bs-toggle="modal" data-bs-target="#caseTransfer_{{ $case->id }}" class="edit-btn btn btn-success waves-effect waves-light" href="{{route("cases.edit",["case"=>$case->id])}}">
                        <span><i class="fa-solid fa-money-bill-transfer me-2"></i> <span
                            class="d-none d-sm-inline-block">@lang('site.transfer')</span></span>
                    </button>
                    @endif
